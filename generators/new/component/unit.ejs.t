...
type TestOverrides = {
	props?: Partial<I<%= h.inflection.camelize(h.changeCase.camelCase(name)) %>>;
};

type TReturn = {
	props: I<%= h.inflection.camelize(h.changeCase.camelCase(name)) %>;
	user: UserEvent;
	container: HTMLElement;
	baseElement: HTMLElement;
};

const getDefaultProps = (overrides: Partial<I<%= h.inflection.camelize(h.changeCase.camelCase(name)) %>> = {}): I<%= h.inflection.camelize(h.changeCase.camelCase(name)) %> => ({
